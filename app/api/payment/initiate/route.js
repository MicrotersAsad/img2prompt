...
export async function POST(request) {
  try {
    console.log('=== Payment API (No Manual Payment) ===');
    ...
    // Get request data
    const body = await request.json();
    const { amount, currency = 'BDT', plan, billing_cycle, order_id } = body;
    console.log('Payment data received:', { amount, currency, plan, billing_cycle, order_id });
    ...
    if (user.subscription?.status === 'active' && user.subscription.plan === plan) {
      return NextResponse.json(
        { success: false, message: 'User already has an active subscription for this plan' },
        { status: 400 }
      );
    }
    ...
    await db.collection('transactions').insertOne({
      ...
      plan: plan,
      billing_cycle: billing_cycle || 'monthly',
      order_id: order_id || null, // Store order_id if provided
      status: 'pending',
      payment_provider: 'piprapay',
      created_at: new Date(),
    });
    ...
    const paymentData = {
      ...
      redirect_url: `${baseUrl}/api/payment/success`,
      webhook_url: 'https://img2prompt-three.vercel.app/api/payment/webhook',
      return_type: 'GET',
      amount: parseFloat(amount).toString(),
      currency: currency,
      order_id: tran_id,
